...
class Searcher(AbstractSearcher):
    ...
    def suggest(self, suggestion_history, n_suggestions=1):
        ...
        """
        next_suggestions = []
        if len(suggestion_history)>n_suggestions*10:#学习阶段
            x_datas, y_datas = self.parse_suggestions_history(suggestion_history)  # 读取数据
            p_names = [p_name for p_name, p_conf in
                       sorted(self.parameters_config.items(), key=lambda x: x[0])]  # 所有的参数名称
            self.tree.fit(x_datas,y_datas) #拟合过程
